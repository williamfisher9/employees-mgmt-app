...
import './WpsEmployer.css'

import MessageBuilder from '../MessageBuilder/MessageBuilder';
import { BACKEND_URL } from '../../constants/constants';


class WpsEmployer extends Component {
    ...
    handleLocalChange = (event) => {
        ...
        if (targetName === "debtorAccountNumber" || targetName === "phoneNumber" || targetName === "paymentYear" || targetName === "paymentMonth") {
            let value = event.target.value;
        ...
        if (targetName === "payerCr" || targetName === "employerCr" || targetName === "deliveryPersonId") {
            let value = event.target.value;
    ...
    handleDateField = (date) => {
        if (date !== null) {
            this.setState({ valueDate: date, paymentMonth: Moment(date).format("MM").toString(), paymentYear: Moment(date).format("YYYY").toString() });
        }
    }
    ...
    validateAndSaveChanges = () => {
        ...
        //employerCr
        if (!this.state.employerCr) {
            formIsValid = false;
            errors["employerCr"] = "Field is required";
        }
        ...
        //payerCr
        if (!this.state.payerCr) {
            formIsValid = false;
            errors["payerCr"] = "Field is required";
        }
        ...
        //paymentMonth
        if (!this.state.paymentMonth) {
            formIsValid = false;
            errors["paymentMonth"] = "Field is required";
        }

        //paymentMonth value
        if (this.state.paymentMonth === 0 || this.state.paymentMonth < 1 || this.state.paymentMonth > 12) {
            formIsValid = false;
            errors["paymentMonth"] = "Enter a valid payment month value";
        }

        //paymentMonth value
        if (this.state.paymentYear === 0 || this.state.paymentYear < 1 || this.state.paymentYear > 2050) {
            formIsValid = false;
            errors["paymentYear"] = "Enter a valid payment year value";
        }

        //paymentYear
        if (!this.state.paymentYear) {
            formIsValid = false;
            errors["paymentYear"] = "Field is required";
        }
                        ...
                        <Row>
                            ...
                            <Col xs={12} xl={6} md={12} className="">
                                <Form.Group controlId="employerName">
                                    ...
                                    <div className="text-left"><Form.Label>Employer Name *</Form.Label></div>
                            ...
                            <Col xs={12} xl={4} md={12} className="">
                                <Form.Group controlId="employerCr">

                                    <div className="text-left"><Form.Label>Employer CR No. *</Form.Label></div>

                                    <Form.Control type="text" name="employerCr"
                                        style={this.state.errors["employerCr"] ? { border: "2px red solid" } : null}
                                        onChange={this.handleLocalChange} value={this.state.employerCr} onBlur={this.trimFunction}
                                        maxLength="17" />

                                    {
                                        this.state.errors["employerCr"] ?
                                            (
                                                <div className="text-left" >
                                                    <span style={{ color: "red", fontSize: "12px" }}>{this.state.errors["employerCr"]}</span>
                                                </div>
                                            )
                                            :
                                            (
                            ...
                            <Col xs={12} xl={4} md={12} className="">
                                <Form.Group controlId="payerCr">
                                    <div className="text-left"><Form.Label>Payer CR No. *</Form.Label></div>
                                    <Form.Control type="text" name="payerCr"
                                        style={this.state.errors["payerCr"] ? { border: "2px red solid" } : null}
                                        onChange={this.handleLocalChange} value={this.state.payerCr} onBlur={this.trimFunction}
                                        maxLength="17" />

                                    {
                                        this.state.errors["payerCr"] ?
                                            (
                                                <div className="text-left" >
                                                    <span style={{ color: "red", fontSize: "12px" }}>{this.state.errors["payerCr"]}</span>
                                                </div>
                                            )
                                            :
                                            (
                            ...
                            <Col xs={12} xl={4} md={12} className="">
                                <Form.Group controlId="paymentType">
                                    ...
                                    <Form.Control as="select" name="paymentType" onChange={this.handleLocalChange} value={this.state.paymentType} >
                                        <option value="101">Salary</option>
                                        <option value="102">Bonus</option>
                                        <option value="104">Allowance</option>
                                        <option value="105">End of service benefit</option>
                                        <option value="103">Overtime Payment</option>
                                    </Form.Control>
                            ...
                            <Col xs={12} xl={4} md={12} className="">
                                <Form.Group controlId="salaryFrequency">
                                    <div className="text-left"><Form.Label>Salary Frequency *</Form.Label></div>
                                    <Form.Control as="select" name="salaryFrequency" onChange={this.handleLocalChange} value={this.state.salaryFrequency} >
                            ...
                            <Col xs={12} xl={4} md={4} className="">
                                <Form.Group controlId="paymentYear">
                                    <div className="text-left"><Form.Label>Payment Year *</Form.Label></div>
                                    <Form.Control type="text" name="paymentYear"
                                        style={this.state.errors["paymentYear"] ? { border: "2px red solid" } : null}
                                        onChange={this.handleLocalChange} value={this.state.paymentYear} onBlur={this.trimFunction}
                                        maxLength="4" />

                                    {
                                        this.state.errors["paymentYear"] ?
                                            (
                                                <div className="text-left" >
                                                    <span style={{ color: "red", fontSize: "12px" }}>{this.state.errors["paymentYear"]}</span>
                                                </div>
                                            )
                                            :
                                            (
                            ...
                            <Col xs={12} xl={4} md={4} className="">
                                <Form.Group controlId="paymentMonth">
                                    <div className="text-left"><Form.Label>Payment Month *</Form.Label></div>
                                    <Form.Control type="text" name="paymentMonth"
                                        style={this.state.errors["paymentMonth"] ? { border: "2px red solid" } : null}
                                        onChange={this.handleLocalChange} value={this.state.paymentMonth} onBlur={this.trimFunction}
                                        maxLength="2" />

                                    {
                                        this.state.errors["paymentMonth"] ?
                                            (
                                                <div className="text-left" >
                                                    <span style={{ color: "red", fontSize: "12px" }}>{this.state.errors["paymentMonth"]}</span>
                                                </div>
                                            )
                                            :
                                            (
