...
import './MinistriesEmployees.css';
import axios from 'axios';
import MinistriesTableRow from '../MinistriesTableRow/MinistriesTableRow'
...
class MinistriesEmployees extends Component {
    ...
    getCompleteRecordsAmount = (inData) => {
        let sum = 0;
        for (let i in inData) {
            sum = parseFloat(sum) + parseFloat(inData[i].amount) - parseFloat(inData[i].deductions);
        }
        return sum;
    }
    ...
    validateItem = (item, employer) => {
        ...
        if (item.deductions < 0) {
            errors.push({ id: item.id, field: "deductions", message: "Min: 0" });
        }
        ...
        if (item.deductions > 999999999.999) {
            errors.push({ id: item.id, field: "deductions", message: "Max: 999999999.999" });
        }

        if ((parseFloat(item.amount) - parseFloat(item.deductions)) <= 0) {
            errors.push({ id: item.id, field: "netAmount", message: "Net > 0" });
        }
    ...
    changeHandler = (event) => {
        ...
        if (targetName === "amount" || targetName === "deductions") {
            let value = event.target.value;
        ...
        if (dataRows[recordIndexInDataRows].id.toString() === itemId.toString()) {
            if (targetName === "accountNumber") {
                ...
                dataRows[recordIndexInDataRows].netAmount = parseFloat(dataRows[recordIndexInDataRows].basicSalary) - parseFloat(dataRows[recordIndexInDataRows].deductions);
    ...
    blurHandler = (event) => {
        ...
        axios.put(`${BACKEND_URL}/api/employees/deductions`, dataRows[recordIndexInDataRows], { headers: { Authorization: `Bearer ${Cookies.get("token")}` }})
            .then(res => {
                if (res.status === 200) {
    ...
    addThousandRecords = () => {
        ...
        axios.post(`${BACKEND_URL}/api/employees/deductions/bulk/${recordsCount}`, { headers: { Authorization: `Bearer ${Cookies.get("token")}` }}).then((res) => {
            if (res.status === 200) {
                this.reloadCompenent(false, {});
            }
        })
...
<div className="d-flex justify-content-between align-items-center actions-container gap-3">
                ...
                <div className="d-flex gap-2">
                        ...
                        <Button style={{ width: "135px", height: "38px" }} variant="outline-dark" className="ml-2 d-inline" onClick={this.validateAndGenerateExcel}>
                            <span style={{}} >Export to Excel</span>
                        </Button>
                    ...
                    <Table responsive="sm" striped hover size="sm">
                        <thead>
                            <tr >
                                ...
                                <th width={'130px'} style={{ textAlign: "middle", fontSize: "11px", verticalAlign: "middle" }}>Employee ID/#</th>
                                ...
                                <th style={{ textAlign: "middle", fontSize: "11px", verticalAlign: "middle" }}>Net Amount</th>
...
export default MinistriesEmployees;